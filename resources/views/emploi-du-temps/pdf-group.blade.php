...
<div >
 ...
 {{-- {{ var_dump($pic) }} --}}
    ...
    <h1 style="font-size: 23px; font-family: Arial, Helvetica, sans-serif; font-weight: bold; text-align: center; margin-bottom: 20px;">Emploi du temps - groupe : {{ $group->nom_group }} </h1>
                                ...
                                @if (isset($filteredSeances[$dayIndex][$intervalIndex]))
                                    @foreach ($filteredSeances[$dayIndex][$intervalIndex] as $seance)
                                        @if (!$chosenGroup || $seance->code_group == $chosenGroup)
                                            ...
                                            Matière: {{ $seance->matiere->label }}<br>
